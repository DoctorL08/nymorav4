...
public class ArenaGenerator : MonoBehaviour
{
    ...
    /// <summary>
    /// True une fois que <see cref="Generate"/> a terminé. Les consommateurs (CombatInitializer
    /// notamment) doivent attendre ce flag avant de lire les cases de spawn ou la grille
    /// car en multijoueur la génération est asynchrone (attente du seed broadcast par le master).
    /// </summary>
    public bool IsArenaReady { get; private set; }

    /// <summary>Préfixe Photon Custom Room Property pour le seed d'arène. Suffixé par le nom de scène
    /// active afin qu'une scène donnée n'écrase pas le seed d'une autre scène (Hub vs Ranked1v1).</summary>
    const string ARENA_SEED_PROP_PREFIX = "arena_seed_";

    /// <summary>Clé spécifique à la scène courante (ex : "arena_seed_Hub", "arena_seed_Ranked1v1").</summary>
    string ArenaSeedKey => ARENA_SEED_PROP_PREFIX + UnityEngine.SceneManagement.SceneManager.GetActiveScene().name;

    void Start()
    {
        if (!generateOnStart) return;

        if (synchronizeWithRoom)
            StartCoroutine(WaitForRoomThenGenerate());
        else
            Generate();
    }

    /// <summary>
    /// Routine d'attente : si on entre dans la scène AVANT d'avoir rejoint la room Photon
    /// (cas du Hub : <c>HubNetworkSpawner</c> joine HUB_GLOBAL de façon asynchrone via
    /// <c>OnConnectedToMaster</c>), on attend d'être InRoom avant de lancer la routine réseau.
    /// Sans cette attente, ArenaGenerator.Start() voyait InRoom=false → branche solo → seed
    /// random local → désync persistante en Hub. En Ranked1v1 c'est immédiat car
    /// <c>PhotonNetwork.LoadLevel</c> maintient la connexion à la matchmaking room.
    /// </summary>
    IEnumerator WaitForRoomThenGenerate()
    {
        const float roomJoinTimeout = 15f;
        float remaining = roomJoinTimeout;

        while (remaining > 0f && !Photon.Pun.PhotonNetwork.InRoom)
        {
        ...
        {
            yield return StartCoroutine(NetworkGenerateRoutine());
        }
        else
        {
            // Pas dans une room après timeout : la scène a été lancée hors flow normal
            // (Editor Play sur la scène seule, mode hors-ligne, etc.) → fallback solo.
            Debug.LogWarning($"[ArenaGenerator] Pas de room Photon après {roomJoinTimeout}s — " +
                             "génération en mode solo. Si tu testais en multi, vérifie la connexion Photon.");
            Generate();
        }
    }

    /// <summary>
    /// Routine multijoueur autoritative master :
    ///   • Le MasterClient pose le seed dans <see cref="Photon.Realtime.Room.CustomProperties"/>
    ///     (s'il n'est pas déjà posé). Photon broadcast la propriété à tous les clients.
    ///   • Tous les clients (master inclus) attendent que la propriété soit lisible localement,
    ///     puis appellent <see cref="Generate"/> avec le même seed → même arène pour tout le monde.
    /// Garantit l'identité bit-à-bit des arènes (mêmes obstacles, mêmes décorations) car
    /// le pipeline déterministe d'<see cref="ArenaGenerator"/> ne dépend que de ce seed.
    /// </summary>
    IEnumerator NetworkGenerateRoutine()
    {
        var room = Photon.Pun.PhotonNetwork.CurrentRoom;
        string key = ArenaSeedKey;

        // Le master pose le seed une seule fois pour la durée de la room (re-match = nouvelle room).
        // Clé spécifique à la scène : permet à plusieurs ArenaGenerator (Hub puis Ranked1v1) de
        // coexister dans la même room sans s'écraser mutuellement.
        if (Photon.Pun.PhotonNetwork.IsMasterClient
            && (room.CustomProperties == null
                || !room.CustomProperties.ContainsKey(key)))
        {
            int seedToBroadcast = arenaConfig.seed >= 0
                ? arenaConfig.seed
            ...
            Debug.Log($"[ArenaGenerator] Master broadcast {key} = {seedToBroadcast}");
        }

        // Tous les clients (master inclus) attendent la propagation locale de la propriété.
        // Côté master c'est immédiat (la copie locale est mise à jour synchroniquement).
        // Côté autres clients : généralement < 100 ms en LAN, < 500 ms en cloud.
        const float timeoutSeconds = 10f;
        float remaining = timeoutSeconds;
        while (remaining > 0f)
        {
            if (room.CustomProperties != null
                && room.CustomProperties.TryGetValue(key, out object raw)
                && raw is int seed)
            {
                Debug.Log($"[ArenaGenerator] Seed réseau reçu : {key}={seed} (master={Photon.Pun.PhotonNetwork.IsMasterClient}).");
                Generate(seed);
                yield break;
            }
            remaining -= Time.deltaTime;
        ...
        Debug.LogError($"[ArenaGenerator] Timeout {timeoutSeconds}s en attente du seed réseau " +
                       $"(propriété {key} jamais reçue). Génération avec seed local — " +
                       "les arènes des deux clients seront probablement désynchronisées.");
        Generate();
    }
    ...
    /// </summary>
    /// <param name="overrideSeed">Si fourni, force ce seed (utilisé par la routine réseau pour
    /// imposer le seed reçu du master, court-circuitant <see cref="ResolveEffectiveSeed"/>).</param>
    public void Generate(int? overrideSeed = null)
    {
        IsArenaReady = false;

        if (!ValidateSetup()) return;

        effectiveSeed = overrideSeed ?? ResolveEffectiveSeed();
    ...
    /// <summary>
    /// Résout le seed effectif pour les modes solo (Training / Editor / Hub seul). En multijoueur
    /// le seed est imposé par <see cref="NetworkGenerateRoutine"/> via override — cette méthode
    /// n'est plus appelée dans ce flow.
    /// </summary>
    int ResolveEffectiveSeed()
    {
        if (arenaConfig.seed >= 0)
            return arenaConfig.seed;
