...
public static class OracleMajGifImportTool
{
    ...
    // Roots where GIF filename alone is the folder name (CombatAnimations, OracleHUD).
    ...
    // Roots where subdirectory name is prefixed to the folder name (Characters/{ClassKey}/anim.gif → ClassKey_anim).
    static readonly (string gifDir, string framesRoot)[] ClassPrefixRoots =
    {
        ("Assets/_Game/Resources/Characters/GifSource", "Assets/_Game/Resources/Characters"),
    };
    ...
    public static void ExtractAll()
    {
        ...
        foreach (var root in ClassPrefixRoots)
        {
            if (!AssetDatabase.IsValidFolder(root.gifDir))
            {
                Debug.LogWarning($"[OracleMajGif] Dossier GIF absent : {root.gifDir}");
            ...
            total += ExtractGifsInTreeWithClassPrefix(root.gifDir, root.framesRoot);
        }
    ...
    static int ExtractGifsInTree(string gifSourceDirAsset, string framesParentAssetPath)
    {
        ...
        foreach (var gifAbs in Directory.GetFiles(absSource, "*.gif", SearchOption.AllDirectories))
        {
            ...
            if (ExtractGifToFolder(gifAbs, absFrames, gifName))
                n++;
        }
        return n;
    }

    // Like ExtractGifsInTree but prefixes the folder/PNG names with the immediate subdirectory name.
    // Characters/GifSource/Ghostra/idle_SO.gif → Frames/Ghostra_idle_SO/Ghostra_idle_SO_000.png
    static int ExtractGifsInTreeWithClassPrefix(string gifSourceDirAsset, string framesParentAssetPath)
    {
        int n = 0;
        string absSource = AssetPathToAbsolute(gifSourceDirAsset);
        if (!Directory.Exists(absSource))
        ...
        {
            string gifName = Path.GetFileNameWithoutExtension(gifAbs);
            string dirAbs  = Path.GetDirectoryName(gifAbs);
            string relDir  = Path.GetRelativePath(absSource, dirAbs);
            string baseName = relDir == "." ? gifName
                : relDir.Replace(Path.DirectorySeparatorChar, '_').Replace('/', '_') + "_" + gifName;

            string relFramesAsset = $"{framesParentAssetPath}/{FramesSubfolder}/{baseName}";
            string absFrames = AssetPathToAbsolute(relFramesAsset);
            Directory.CreateDirectory(absFrames);
            if (ExtractGifToFolder(gifAbs, absFrames, baseName))
                n++;
        }
        return n;
    }
    ...
    static bool ExtractGifToFolder(string gifAbsPath, string outDirAbs, string baseName = null)
    {
        if (baseName == null) baseName = Path.GetFileNameWithoutExtension(gifAbsPath);
        try
        {
            ...
            for (int i = 0; i < frameCount; i++)
            {
                ...
                string pngAbs = Path.Combine(outDirAbs, $"{baseName}_{i:D3}.png");
        ...
        {
            Debug.LogError($"[OracleMajGif] {baseName} : {ex.Message}");
            return false;
        }
    }
