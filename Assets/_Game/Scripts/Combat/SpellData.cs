...
public enum SpellEffectType
{
    ...
    Bleed,               // N dégâts par tour pendant D tours — Ghostra (Hémorragie)
    Brulure,             // Soulrender — Brûlure stackable (même logique que Bleed)
    ...
    /// <summary>Soigne le LANCEUR d'un montant fixe (indépendamment de la cible du sort). Utilisé pour le drain vital.</summary>
    HealCaster,
    /// <summary>Necram — Applique 1 marque de venin (Bleed adapté au palier Toxine) à la cible. La valeur de tick vient du PassiveManager.</summary>
    ApplyVenin,
    /// <summary>Necram — Consomme toutes les marques de venin sur la cible.
    /// value&gt;0 : bonus plat par marque (ex. 28), duration = cap total (ex. 130) → Détonation Virulente.
    /// value=0 : déclenche tous les ticks restants × multiplicateur Enragé → Virus Fatal.</summary>
    ConsumeVenin,
}
...
public enum SpellCondition
{
    ...
    AtExactCastDistance,
    /// <summary>Necram — Bonus si la cible a au moins 1 marque de venin active (Bleed stack).</summary>
    TargetHasVenin,
}
