...
public class TacticalCharacter : MonoBehaviour
{
    ...
    public event System.Action OnSpellsChanged;

    public void ClearRuntimeSpellDeck() => runtimeSpells.Clear();

    /// <summary>Ajoute un sort en dehors du deck normal (sort Enragé, bonus passif…) et notifie l'UI.</summary>
    public void AddRuntimeSpell(SpellData spell)
    {
        if (spell == null || runtimeSpells.Contains(spell)) return;
        if (runtimeSpells.Count == 0 && deck != null)
            runtimeSpells.AddRange(deck.Spells);
        runtimeSpells.Add(spell);
        OnSpellsChanged?.Invoke();
    }

    /// <summary>Retire un sort du deck runtime (ex. Saignée après utilisation) et notifie l'UI.</summary>
    public void RemoveRuntimeSpell(SpellData spell)
    {
        if (spell == null || !runtimeSpells.Contains(spell)) return;
        runtimeSpells.Remove(spell);
        OnSpellsChanged?.Invoke();
    }
    ...
    public void AddStatusEffect(StatusEffect effect)
    {
        if (effect.type == StatusEffectType.Bleed ||
            effect.type == StatusEffectType.Venin  ||
            effect.type == StatusEffectType.Brulure)
        {
            // Stacking réel : chaque application est une instance indépendante
            activeEffects.Add(effect);
            return;
        }
        if (effect.type == StatusEffectType.Shield)
        {
            var existing = activeEffects.Find(e => e.type == StatusEffectType.Shield);
            if (existing != null)
            {
    ...
    public void RemoveAllVeninEffects() =>
        activeEffects.RemoveAll(e => e.type == StatusEffectType.Venin);

    public void RemoveAllBrulureEffects() =>
        activeEffects.RemoveAll(e => e.type == StatusEffectType.Brulure);

    public int CountBrulureStacks()
    {
        int count = 0;
        foreach (var e in activeEffects)
            if (e.type == StatusEffectType.Brulure) count++;
        return count;
    }

    /// <summary>Nombre de marques de Venin (Necram/Toxine) actives sur ce personnage.</summary>
    public int CountVeninStacks()
    {
        int count = 0;
        foreach (var e in activeEffects)
            if (e.type == StatusEffectType.Venin) count++;
        return count;
    }

    /// <summary>Necram — Virus Fatal : somme plate des valeurs de chaque marque (indépendant des tours restants), puis retire les marques.</summary>
    public int ConsumeAllVenin()
    {
        int total = 0;
        foreach (var e in activeEffects)
            if (e.type == StatusEffectType.Venin)
                total += e.value;
        RemoveAllVeninEffects();
        return total;
    }
    ...
    private void ProcessTurnStartEffects()
    {
        ...
        foreach (var effect in activeEffects)
        {
            if (effect.type == StatusEffectType.Bleed)
            {
                ...
                totalBleedDmg += TakeDamage(effect.value, null, suppressFloatingText: true);
            }
            else if (effect.type == StatusEffectType.Venin)
            {
                CombatLog.Append($"<b>{name}</b> est empoisonné <color=#88FF44>-{effect.value}</color>");
                totalBleedDmg += TakeDamage(effect.value, null, suppressFloatingText: true);
            }
            else if (effect.type == StatusEffectType.Brulure)
            {
                CombatLog.Append($"<b>{name}</b> brûle <color=#FF8844>-{effect.value}</color>");
                totalBleedDmg += TakeDamage(effect.value, null, suppressFloatingText: true);
            }

            // Ces effets sont décrémentés en fin de tour (ProcessTurnEndEffects) — ne pas double-ticker
            if (effect.type == StatusEffectType.Silence        ||
                effect.type == StatusEffectType.DamageReduction||
        ...
        if (totalBleedDmg > 0)
            FloatingDamageText.SpawnDamage(totalBleedDmg, transform.position);
    ...
    public void SpendPA(int amount)
    {
        ...
        OnPAChanged?.Invoke(currentPA, stats.maxPA);
        if (amount > 0)
            FloatingDamageText.SpawnPACost(amount, transform.position);
    }
