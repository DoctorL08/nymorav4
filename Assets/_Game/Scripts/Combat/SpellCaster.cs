...
public class SpellCaster : MonoBehaviour
{
    ...
    /// <summary>
    /// Calcule les dégâts/soins que <paramref name="spell"/> infligerait à <paramref name="target"/>,
    /// en tenant compte des passifs et de l'état actuel (position, debuffs, etc.).
    /// </summary>
    public static (int dmg, int heal, int shield) ComputePreview(SpellData spell, TacticalCharacter caster, TacticalCharacter target)
    {
        int dmg = 0, heal = 0, shield = 0;
        if (spell == null || caster == null || target == null) return (0, 0, 0);

        int castDist = 0;
        if (caster.CurrentCell != null && target.CurrentCell != null)
            castDist = Mathf.Abs(caster.CurrentCell.GridX - target.CurrentCell.GridX)
                     + Mathf.Abs(caster.CurrentCell.GridY - target.CurrentCell.GridY);
        ...
        foreach (SpellEffect effect in spell.effects)
        {
            switch (effect.type)
            {
                case SpellEffectType.Damage:
                    ...
                    else if (effect.condition == SpellCondition.SelfHPBelow)
                    {
                        if (caster.stats == null) continue;
                        int th = Mathf.Max(1, Mathf.RoundToInt(caster.stats.maxHP * (effect.conditionThreshold / 100f)));
                        if (caster.CurrentHP >= th) continue;
                    }
                    else if (effect.condition == SpellCondition.FromBehind)
                    {
                        ...
                        continue;
                    else if (effect.condition == SpellCondition.TargetHasDebuff && !target.HasAnyDebuff())
                        continue;
                    else if (effect.condition == SpellCondition.AtMaxCastRange)
                    {
                        int effMax = spell.rangeMax + caster.GetBonusRange();
                        if (caster.CurrentCell == null || target.CurrentCell == null) continue;
                        if (castDist != effMax) continue;
                    }
                    ...
                    {
                        if (castDist != effect.conditionThreshold) continue;
                    }
                    dmg += d;
                    break;
                ...
                case SpellEffectType.Heal:
                    if (effect.condition == SpellCondition.SelfHPBelow)
                    {
                        if (caster.stats == null) continue;
                        int th = Mathf.Max(1, Mathf.RoundToInt(caster.stats.maxHP * (effect.conditionThreshold / 100f)));
                        if (caster.CurrentHP >= th) continue;
        ...
        // Appliquer le passif du lanceur (lecture seule, sans effets de bord)
        if (dmg > 0)
        {
            var pm = caster.GetComponent<PassiveManager>();
            if (pm != null)
                dmg = pm.ModifyOutgoingDamagePreview(dmg, spell, target, castDist);
        }

        return (dmg, heal, shield);
    }
}
