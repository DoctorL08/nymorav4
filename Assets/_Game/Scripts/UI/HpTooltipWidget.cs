...
#pragma warning restore CS0162
    ...
    void RefreshPreview(TacticalCharacter tc)
    {
        ...
        bool any = dmg > 0 || heal > 0 || shieldGain > 0;
        if (!any) { _previewText.gameObject.SetActive(false); return; }

        var parts = new System.Text.StringBuilder();
        if (dmg > 0)        parts.Append($"<color=#FF6060>- {dmg} dégâts</color>");
        if (heal > 0)       { if (parts.Length > 0) parts.Append("  |  "); parts.Append($"<color=#6CFF85>+ {heal} soin</color>"); }
        if (shieldGain > 0) { if (parts.Length > 0) parts.Append("  |  "); parts.Append($"<color=#73CCFF>+ {shieldGain} bouclier</color>"); }

        _previewText.text  = parts.ToString();
