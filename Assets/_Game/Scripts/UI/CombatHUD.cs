...
public class CombatHUD : MonoBehaviour
{
    ...
    GaugeHUDWidget _gaugeWidgetA;
    GaugeHUDWidget _gaugeWidgetB;
    ...
    void BuildGaugeWidgets()
    {
        var canvas = GetComponentInParent<Canvas>();
        if (canvas == null) canvas = FindObjectOfType<Canvas>();
        if (canvas == null) return;

        var goA = new GameObject("GaugeWidgetA", typeof(RectTransform));
        goA.transform.SetParent(canvas.transform, false);
        _gaugeWidgetA = goA.AddComponent<GaugeHUDWidget>();
        _gaugeWidgetA.Init(alignRight: false);

        var goB = new GameObject("GaugeWidgetB", typeof(RectTransform));
        goB.transform.SetParent(canvas.transform, false);
        _gaugeWidgetB = goB.AddComponent<GaugeHUDWidget>();
        _gaugeWidgetB.Init(alignRight: true);

        if (teamACharacter != null) _gaugeWidgetA.SetCharacter(teamACharacter);
        if (teamBCharacter != null) _gaugeWidgetB.SetCharacter(teamBCharacter);
    }
    ...
    IEnumerator LateHpRefresh()
    {
        ...
        for (int attempt = 0; attempt < 30; attempt++)
        {
            ...
            if (_subHP_A == null && teamACharacter != null && teamACharacter.stats != null)
            {
                ...
                if (_gaugeWidgetA != null && _gaugeWidgetA.Character == null)
                    _gaugeWidgetA.SetCharacter(teamACharacter);
            }
            if (_subHP_B == null && teamBCharacter != null && teamBCharacter.stats != null)
            {
                ...
                if (_gaugeWidgetB != null && _gaugeWidgetB.Character == null)
                    _gaugeWidgetB.SetCharacter(teamBCharacter);
            }
