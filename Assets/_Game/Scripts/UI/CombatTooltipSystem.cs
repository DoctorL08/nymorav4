...
public class CombatTooltipSystem : MonoBehaviour
{
    ...
    void ShowPanel(Panel p, TacticalCharacter tc, SpellCaster sc, TacticalCharacter caster)
    {
        ...
        // Bouclier
        int shield = tc.ShieldHP;
        if (shield > 0)
        {
            p.shieldText.text = $"Bouclier : {shield}";
            p.shieldText.gameObject.SetActive(true);
        }
        else
        {
            p.shieldText.gameObject.SetActive(false);
        }

        // Prévisualisation (seulement si sort actif passé en paramètre)
        if (sc != null && sc.SelectedSpell != null)
        {
            var (dmg, heal, shieldGain) = SpellCaster.ComputePreview(sc.SelectedSpell, caster, tc);

            // PA/PM : lecture directe des effets du sort
            int removePM = 0, gainPA = 0, gainPM = 0;
            foreach (var e in sc.SelectedSpell.effects)
            {
                switch (e.type)
                {
                    case SpellEffectType.RemovePM:
                    case SpellEffectType.StealPM:
                        removePM += e.value; break;
                    case SpellEffectType.BonusPA:
                    case SpellEffectType.BonusPANextTurn:
                        gainPA += e.value; break;
                    case SpellEffectType.BonusPM:
                        gainPM += e.value; break;
                }
            }

            bool any = dmg > 0 || heal > 0 || shieldGain > 0 || removePM > 0 || gainPA > 0 || gainPM > 0;
            if (any)
            {
                var parts = new System.Text.StringBuilder();
                if (dmg > 0)
                    parts.Append($"<color=#FF6060>- {dmg} dégâts</color>");
                if (heal > 0)
                {
                    if (parts.Length > 0) parts.Append("  |  ");
                    parts.Append($"<color=#6CFF85>+ {heal} soin</color>");
                }
                if (shieldGain > 0)
                {
                    if (parts.Length > 0) parts.Append("  |  ");
                    parts.Append($"<color=#73CCFF>+ {shieldGain} bouclier</color>");
                }
                if (removePM > 0)
                {
                    if (parts.Length > 0) parts.Append("  |  ");
                    parts.Append($"<color=#88AAFF>- {removePM} PM</color>");
                }
                if (gainPA > 0)
                {
                    if (parts.Length > 0) parts.Append("  |  ");
                    parts.Append($"<color=#FFD700>+ {gainPA} PA</color>");
                }
                if (gainPM > 0)
                {
                    if (parts.Length > 0) parts.Append("  |  ");
                    parts.Append($"<color=#88AAFF>+ {gainPM} PM</color>");
                }
                p.previewText.text  = parts.ToString();
                p.previewText.color = Color.white;
    ...
    Panel BuildPanel()
    {
        ...
        var shieldT  = MakeLabel(go.transform, "Shield",  11f, FontStyles.Normal, new Color(0.45f,  0.80f,  1.00f));
        ...
        return new Panel { root = rt, nameText = nameT, hpText = hpT, shieldText = shieldT, previewText = previewT };
    }
