...
public class HubPlayerAvatar : MonoBehaviourPunCallbacks
{
    ...
    void Awake()
    {
        ...
        if (photonView.IsMine)
        {
            ...
            HubManager.OnClassChanged += HandleLocalClassChanged;
        }
        else
        {
        ...
        // Charger la classe embarquée dans l'instantiationData (tous avatars, IsMine et !IsMine)
        StartCoroutine(ApplyInstantiationDataNextFrame());

        // Masquer immédiatement si l'avatar est instancié hors du Hub
        ApplyVisibility();
    }
    ...
    {
        bool inHub = SceneManager.GetActiveScene().name == HUB_SCENE_NAME;

        // Masquer le label Canvas (GameObject séparé) avant de désactiver le GameObject
        if (_nameLabel != null)
            _nameLabel.transform.parent.gameObject.SetActive(inHub);

        if (gameObject.activeSelf != inHub)
            gameObject.SetActive(inHub);
    }

    // ── Sync de la classe via RPC bufferisé ───────────────────────────────

    IEnumerator ApplyInstantiationDataNextFrame()
    {
        yield return null; // attendre que PlayerAnimator.Start() s'exécute
        var data = photonView.InstantiationData;
        if (data != null && data.Length > 0 && data[0] is int classIdInt)
        {
            var cls = HubManager.LoadClassById((NymoraClassId)classIdInt);
            if (cls != null && !string.IsNullOrEmpty(cls.characterKey))
            {
                var animator = GetComponent<PlayerAnimator>();
                if (animator != null) animator.LoadCharacterAnimations(cls.characterKey);
            }
        }
    }

    void HandleLocalClassChanged(ClassData cls)
    {
        if (!photonView.IsMine) return;
        BroadcastLocalClass();
    }

    void BroadcastLocalClass()
    {
        var cls = HubManager.Instance?.SelectedClass;
        if (cls == null) return;
        photonView.RPC(nameof(RpcSetHubClassId), Photon.Pun.RpcTarget.AllBuffered, (int)cls.classId);
    }

    [PunRPC]
    void RpcSetHubClassId(int classIdInt)
    {
        var cls = HubManager.LoadClassById((NymoraClassId)classIdInt);
