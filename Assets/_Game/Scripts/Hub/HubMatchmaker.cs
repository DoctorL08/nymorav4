...
public class HubMatchmaker : MonoBehaviourPunCallbacks
{
    ...
    void TryStartCombat()
    {
        ...
        // Classe résolue via Photon CustomProperties pour éviter la désync inter-clients.
        var localClass = HubManager.Instance?.SelectedClass;
        if (localClass != null)
        {
            var props = new ExitGames.Client.Photon.Hashtable { { "classId", (int)localClass.classId } };
