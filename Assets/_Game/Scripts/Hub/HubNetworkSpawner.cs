...
public class HubNetworkSpawner : MonoBehaviourPunCallbacks
{
    ...
    void OnSceneLoaded(Scene scene, LoadSceneMode mode)
    {
        if (_myPlayer != null)
        {
            if (scene.name != "Hub")
            {
                // PhotonNetwork.Destroy nécessaire car HubPlayer est un objet réseau.
                if (PhotonNetwork.IsConnected && _myPlayer.GetComponent<PhotonView>()?.IsMine == true)
                    PhotonNetwork.Destroy(_myPlayer);
                else
                    Destroy(_myPlayer);
            }
    ...
    public override void OnJoinedRoom()
    {
        ...
        // Diffuser la classe sélectionnée AVANT le spawn pour que les autres clients
        // construisent l'avatar distant avec la bonne classe dès le premier frame.
        HubManager.PushSelectedClassToPhoton();
    ...
    void SpawnPlayer()
    {
        ...
        var localCls = HubManager.Instance?.SelectedClass;
        int classIdInt = localCls != null ? (int)localCls.classId : (int)NymoraClassId.Ghostra;
        object[] data = new object[] { classIdInt };
        _myPlayer = PhotonNetwork.Instantiate(avatarPrefabName, Vector3.zero, Quaternion.identity, 0, data);
    }
}
